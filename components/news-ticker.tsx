...
export default function NewsTicker() {
  ...
  const NewsElement = ({ item, index }: { item: NewsItem; index: number }) => {
    ...
    const baseClasses = `
      ...
      ${item.priority === "breaking" ? "bg-gradient-to-r from-red-900/80 to-red-950/70" : "bg-gradient-to-r from-black/70 to-zinc-900/60"} 
      ${item.link ? "cursor-pointer" : "cursor-default"}
      rounded-sm border ${item.priority === "breaking" ? "border-red-500/30" : "border-white/10"}
  ...
  return (
    <div 
      className="w-full h-[0.6in] overflow-hidden relative z-20 bg-gradient-to-r from-red-950/60 via-black/70 to-red-950/60 backdrop-blur-sm shadow-lg"
      ref={tickerRef}
      onMouseEnter={() => {
        setIsPaused(true);
        setHasInteracted(true);
