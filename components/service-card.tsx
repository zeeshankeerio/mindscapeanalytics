...
export default function ServiceCard({ title, description, icon, features = [], image }: ServiceCardProps) {
  ...
  return (
    ...
    >
      <Card className="h-full overflow-hidden group bg-background/10 backdrop-blur-sm shadow-xl transition-all duration-300 relative border-0 hover:shadow-2xl">
        <div className="absolute inset-0 bg-gradient-to-br from-primary/10 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
        
        <div className="absolute inset-0 opacity-10 group-hover:opacity-20 transition-opacity duration-500">
          <Image
            src={!imgError && image ? image : "/images/services/service-placeholder.svg"}
            alt={title}
            fill
        ...
        <CardHeader className="pb-2 relative z-10">
          ...
          >
            ...
            <div className="absolute -inset-4 rounded-full bg-primary/10 scale-0 group-hover:scale-100 transition-transform duration-300"></div>
          </motion.div>
          <CardTitle className="text-xl bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary/80">{title}</CardTitle>
        </CardHeader>
          ...
          {features.length > 0 && (
            <ul className="space-y-2 mb-4">
              {features.map((feature, index) => (
                ...
                >
                  <div className="h-1.5 w-1.5 rounded-full bg-primary shadow-sm shadow-primary/20"></div>
